    ...
    function draw() {
      ...
      stars.forEach(s => {
        ...
        if (s.r > 0.8) {
          ...
          ctx.fillStyle = `rgba(180,180,255,${a * 0.1})`
          ctx.fill()
        }
      })
      ...
      <div className="home-content">
        ...
        {tasksToday.length > 0 ? (
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ background: 'var(--bg2)', borderRadius: 14, padding: '4px 14px', border: '0.5px solid rgba(96,165,250,0.15)' }}>
              {tasksToday.map(task => (
...
function HabitRow({ habit, recoverable, onToggle, onRecover }) {
  ...
  return (
    <div style={{
      ...
      padding: '10px 14px',
      background: doneToday ? 'rgba(249,115,22,0.07)' : 'var(--bg2)',
      borderRadius: 12,
      border: `0.5px solid ${doneToday ? 'rgba(249,115,22,0.35)' : 'rgba(255,255,255,0.05)'}`,
      transition: 'all 0.2s',
    }}>
      <button
        onClick={onToggle}
